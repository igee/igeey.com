...
var DateTimeShortcuts = {
    ...
    init: function() {
        ...
        for (i=0; i<inputs.length; i++) {
            var inp = inputs[i];
            if (inp.getAttribute('type') == 'text' && inp.className.match(/with_clock/)) {
                DateTimeShortcuts.addClock(inp);
            }
            else if (inp.getAttribute('type') == 'text' && inp.className.match(/with_calendar/)) {
    ...
    addCalendar: function(inp) {
        ...
        $(inp).click(function(){return false})
        $(inp).focus(function(){DateTimeShortcuts.openCalendar(num);})
